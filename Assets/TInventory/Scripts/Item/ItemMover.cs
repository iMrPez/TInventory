    ...
    public class ItemMover : MonoBehaviour
    {
        ...
        private bool IsAttemptingToHoldItem()
        {
            if (InputHandler.GetPrimaryButtonDown())
            {
                ...
                // Quick move item if modifier button is clicked and an inventory is set
                if (InputHandler.GetModifierButton() && !(Inventory.Instance.InventoryWindow is null))
                {
                    (Vector2 slot, ContainerGroup group) target;
                    foreach (var container in Inventory.Instance.InventoryWindow.Containers)
                    {
                        if (container.CanAddItem(_clickedItem, out target))
                        {
                            container.PlaceItemAt(target.slot, target.group, _clickedItem);
                        }
                    }
                }
